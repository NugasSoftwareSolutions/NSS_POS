...
{
    public partial class frmLoadWalletTransactionsReport : Form
    {
        List<clsUsers> lstUsers = new List<clsUsers>();
        List<clsLoadAccount> m_lstAccountInfo = new List<clsLoadAccount>();

        public frmLoadWalletTransactionsReport()
        {
            InitializeComponent();
        }

        private void frmLoadWalletTransactionsReport_Load(object sender, EventArgs e)
        {
            ...
            {
                if (acc.LoadType == LoadAccountType.LoadWallet)
                {
        ...
        private void SearchEloadTrans(DateTime startdate, DateTime enddate, int cashier, int loadid)
        {
            ...
            List<clsLoadWalletTransaction> lstLoadHistory = clsLoadWalletTransaction.GetLoadWalletTransactionsReport(startdate, enddate, cashier, loadid);
            dgvTrans.Rows.Clear();
            if (lstLoadHistory.Count > 0)
            {
                foreach (clsLoadWalletTransaction hist in lstLoadHistory)
                {
                    AddItemToGrid(hist);
                }
            }
        ...
        private void frmLoadWalletTransactionsReport_FormClosing(object sender, FormClosingEventArgs e)
        {
            e.Cancel = true;
            this.Hide();
        }
